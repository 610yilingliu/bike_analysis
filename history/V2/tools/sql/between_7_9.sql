...
select top 1 * from (select convert(time, UPDATE_TIME) as dtime, * from bike_detail) as t1 where t1.LOCK_STATUS = 1 order by t1.dtime asc;
select top 1 * from (select convert(time, UPDATE_TIME) as dtime, * from bike_detail) as t1 where t1. LOCK_STATUS = 0 order by t1.dtime desc;
select BID, BICYCLE_ID, LATITUDE, LONGITUDE, LOCK_STATUS, convert(smalldatetime, UPDATE_TIME) as UPDATE_TIME, GRID, NEAREST_FENCE, DISTANCE from (select *, datepart(hh, UPDATE_TIME) as happen_hour from bike_detail) as t1
